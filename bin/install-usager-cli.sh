...
HELPER="$APP/Contents/Helpers/UsagerCLI"
TARGETS=("/usr/local/bin/usager" "/opt/homebrew/bin/usager")

if [[ ! -x "$HELPER" ]]; then
  echo "UsagerCLI helper not found at $HELPER. Please reinstall Usager." >&2
  exit 1
fi
...
on run argv
  ...
  set installCommand to "set -euo pipefail" & linefeed & ¬
    ...
    "TARGETS=(\"/usr/local/bin/usager\" \"/opt/homebrew/bin/usager\")" & linefeed & ¬
...
echo "Usager CLI installed. Try: usager usage"
