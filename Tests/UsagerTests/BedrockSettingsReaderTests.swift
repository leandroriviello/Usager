...
struct BedrockSettingsReaderTests {
    ...
    func `explicit profile auth mode wins`() {
        let env = ["USAGER_BEDROCK_AUTH_MODE": "profile"]
    ...
    func `hasCredentials in profile mode requires a profile name`() {
        let withProfile = ["USAGER_BEDROCK_AUTH_MODE": "profile", "AWS_PROFILE": "work"]
        let withoutProfile = ["USAGER_BEDROCK_AUTH_MODE": "profile"]
