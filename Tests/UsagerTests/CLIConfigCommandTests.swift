...
struct CLIConfigCommandTests {
    ...
    func `config set api key parses provider stdin and no enable flags`() throws {
        let parser = CommandParser(signature: UsagerCLI._configSetAPIKeySignatureForTesting())
    ...
    func `config set api key stores key and enables provider`() {
        ...
        let updated = UsagerCLI.configSettingAPIKey(
            config,
            provider: .elevenlabs,
    ...
    func `config set api key stores zai team token account`() throws {
        let config = UsagerConfig.makeDefault()
        let options = try UsagerCLI.resolveConfigAPIKeyAccountOptions(
            provider: .zai,
            label: "Team",
            usageScope: "team",
    ...
    func `config set api key rejects incomplete zai team account options`() {
        #expect(throws: CLIArgumentError.self) {
            _ = try UsagerCLI.resolveConfigAPIKeyAccountOptions(
                provider: .zai,
                label: "Team",
                usageScope: "team",
                organizationID: "org-team",
    ...
    func `config provider toggle parses provider and json flags`() throws {
        let parser = CommandParser(signature: UsagerCLI._configProviderToggleSignatureForTesting())
        let parsed = try parser.parse(arguments: [
            "--provider", "grok",
            "--json",
            "--pretty",
    ...
    func `config provider toggle enables and disables provider`() {
        let config = UsagerConfig.makeDefault()
        let enabled = UsagerCLI.configSettingProviderEnabled(config, provider: .grok, enabled: true)
        let disabled = UsagerCLI.configSettingProviderEnabled(enabled, provider: .grok, enabled: false)
    ...
    func `config provider status includes effective default`() throws {
        let config = UsagerConfig(providers: [
            ProviderConfig(id: .grok, enabled: true),
            ProviderConfig(id: .cursor, enabled: false),
        ])
        let statuses = UsagerCLI.configProviderStatuses(config)
    ...
    func `config set api key preserves disabled provider when requested`() {
        var config = UsagerConfig.makeDefault()
    ...
    func `config set api key rejects ambiguous input`() {
        #expect(throws: CLIArgumentError.self) {
            try UsagerCLI.resolveConfigAPIKeyInput(apiKey: "xi-test-token", readFromStdin: true)
        }
    }

    @Test
    func `config help documents set api key`() {
        let help = UsagerCLI.configHelp(version: "0.0.0")
