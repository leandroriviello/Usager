    ...
    func `fetch usage sets credits timeout and client headers`() async throws {
        ...
        OpenRouterStubURLProtocol.handler = { request in
            ...
            case "/api/v1/credits":
                ...
                #expect(request.value(forHTTPHeaderField: "HTTP-Referer") == "https://usager.example")
                #expect(request.value(forHTTPHeaderField: "X-Title") == "Usager QA")
        ...
        let usage = try await OpenRouterUsageFetcher.fetchUsage(
            ...
            environment: [
                ...
                "OPENROUTER_HTTP_REFERER": " https://usager.example ",
                "OPENROUTER_X_TITLE": "Usager QA",
            ])
