...
struct ClaudeOAuthCredentialsStoreTests {
    ...
    @Test
    func `load with auto refresh expired usager owner uses direct refresh path`() async throws {
        let service = "com.leandroriviello.usager.cache.tests.\(UUID().uuidString)"
        try await KeychainCacheStore.withServiceOverrideForTesting(service) {
            KeychainCacheStore.setTestStoreForTesting(true)
            defer { KeychainCacheStore.setTestStoreForTesting(false) }
            ...
            await ClaudeOAuthCredentialsStore.withCredentialsURLOverrideForTesting(fileURL) {
                await ClaudeOAuthCredentialsStore.withKeychainAccessOverrideForTesting(true) {
                    ...
                    let expiredData = self.makeCredentialsData(
                        accessToken: "expired-usager-owner",
                        expiresAt: Date(timeIntervalSinceNow: -3600),
                        refreshToken: "refresh-token")
                    KeychainCacheStore.store(
                        key: cacheKey,
                    ...
                    await ClaudeOAuthRefreshFailureGate.$shouldAttemptOverride.withValue(false) {
                        do {
                            _ = try await ClaudeOAuthCredentialsStore.loadWithAutoRefresh(
                                ...
                                allowKeychainPrompt: false,
                                respectKeychainPromptCooldown: true)
                            Issue.record("Expected refresh failure for Usager-owned direct refresh path")
                        } catch let error as ClaudeOAuthCredentialsError {
    ...
    @Test
    func `has cached credentials returns false for expired unrefreshable usager cache entry`() throws {
        KeychainCacheStore.setTestStoreForTesting(true)
        defer { KeychainCacheStore.setTestStoreForTesting(false) }
