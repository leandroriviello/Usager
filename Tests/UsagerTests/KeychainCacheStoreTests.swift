...
struct KeychainCacheStoreTests {
    ...
    func `tests suppress real keychain access by default`() {
        guard ProcessInfo.processInfo.environment["USAGER_ALLOW_TEST_KEYCHAIN_ACCESS"] != "1" else { return }
    ...
    func `cache ACL trusts bundled app and CLI helper`() {
        let root = URL(fileURLWithPath: "/Applications/Usager.app")
        let executable = root.appendingPathComponent("Contents/MacOS/Usager")
        let helper = root.appendingPathComponent("Contents/Helpers/UsagerCLI")
        let existing = Set([
            root.path,
            executable.path,
            helper.path,
