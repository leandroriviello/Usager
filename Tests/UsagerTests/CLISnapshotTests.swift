...
struct CLISnapshotTests {
    ...
    func `defaults to usage when no command provided`() {
        #expect(UsagerCLI.effectiveArgv([]) == ["usage"])
        #expect(UsagerCLI.effectiveArgv(["--format", "json"]).first == "usage")
        #expect(UsagerCLI.effectiveArgv(["usage", "--format", "json"]).first == "usage")
    }
