...
struct TTYCommandRunnerEnvTests {
    ...
    func `codex status probe uses non persistent thread storage`() {
        let stateHome = URL(fileURLWithPath: "/tmp/usager status \"state\"", isDirectory: true)
        ...
        #expect(args.contains("experimental_thread_store={type=\"in_memory\",id=\"usager-status\"}"))
        #expect(args.contains("sqlite_home=\"/tmp/usager status \\\"state\\\"\""))
    }
    ...
    func `sets working directory when provided`() throws {
        ...
        let dir = fm.temporaryDirectory.appendingPathComponent("usager-tty-\(UUID().uuidString)", isDirectory: true)
