...
struct CostUsageScannerBreakdownTests {
    ...
    private func makeGitRepositoryWithWorktree(projectPath: String, worktreePath: String) throws {
        ...
        try self.runGit(["-C", projectPath, "config", "user.email", "usager-test@example.com"])
        try self.runGit(["-C", projectPath, "config", "user.name", "Usager Test"])
