...
struct StatusItemControllerSplitLifecycleTests {
    ...
    func `merged mode removes split provider status items`() throws {
        ...
        #expect(controller.expectedVisibleStatusItemAutosaveNames == ["usager-codex", "usager-claude"])
        ...
        #expect(controller.expectedVisibleStatusItemAutosaveNames == ["usager-merged"])
    }
    ...
    func `status items publish stable manager identity`() throws {
        ...
        #expect(controller.statusItem.autosaveName == "usager-merged")
        #expect(controller.statusItems[.codex]?.autosaveName == "usager-codex")
        #expect(controller.statusItems[.claude]?.autosaveName == "usager-claude")
        #expect(controller.statusItem.button?.accessibilityIdentifier() == "Usager.StatusItem")
        #expect(codexButton.accessibilityIdentifier() == "Usager.StatusItem.codex")
        #expect(claudeButton.accessibilityIdentifier() == "Usager.StatusItem.claude")
        #expect(controller.statusItem.button?.accessibilityTitle() == "Usager")
        #expect(codexButton.accessibilityTitle() == "Usager")
        #expect(claudeButton.accessibilityTitle() == "Usager")
    }

    @Test
    func `status item identity returns stable autosave names`() {
        #expect(StatusItemController.StatusItemIdentity.merged.autosaveName == "usager-merged")
        #expect(StatusItemController.StatusItemIdentity.provider(.codex).autosaveName == "usager-codex")
        #expect(StatusItemController.StatusItemIdentity.provider(.claude).autosaveName == "usager-claude")
    }
    ...
    func `status item placement preflight leaves fresh install placement unset`() throws {
        ...
        let key = MenuBarStatusItemPlacementPreflight.preferredPositionKey(autosaveName: "usager-merged")

        #expect(!MenuBarStatusItemPlacementPreflight.prepare(defaults: defaults, autosaveName: "usager-merged"))
    ...
    func `status item placement preflight clears provider matching suspicious legacy placement`() throws {
        ...
        defaults.set(42, forKey: "NSStatusItem Preferred Position Item-0")
        defaults.set(11298, forKey: "NSStatusItem Preferred Position Item-1")
        let key = MenuBarStatusItemPlacementPreflight.preferredPositionKey(autosaveName: "usager-codex")

        #expect(MenuBarStatusItemPlacementPreflight.prepare(
            defaults: defaults,
            autosaveName: "usager-codex",
    ...
    func `status item defaults repair removes stale hidden Control Center keys once`() throws {
        ...
        defaults.set(false, forKey: "NSStatusItem VisibleCC usager-merged")
        ...
        #expect(repairedKeys == [
            ...
            "NSStatusItem VisibleCC usager-merged",
        ])
        ...
        #expect(defaults.object(forKey: "NSStatusItem VisibleCC usager-merged") == nil)
    ...
    func `status item visibility default distinguishes enabled disabled and unset`() throws {
        ...
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        defaults.set(true, forKey: "NSStatusItem VisibleCC usager-merged")
        defaults.set(false, forKey: "NSStatusItem VisibleCC usager-claude")
        defer { defaults.removePersistentDomain(forName: suite) }

        #expect(MenuBarStatusItemDefaultsRepair.visibilityDefault(
            defaults: defaults,
            autosaveName: "usager-merged") == true)
        #expect(MenuBarStatusItemDefaultsRepair.visibilityDefault(
            defaults: defaults,
            autosaveName: "usager-claude") == false)
        #expect(MenuBarStatusItemDefaultsRepair.visibilityDefault(
            defaults: defaults,
            autosaveName: "usager-codex") == nil)
    }
    ...
    func `non destructive visibility refresh preserves split provider status items`() throws {
        ...
        #expect(newCodexItem.autosaveName == "usager-codex")
        #expect(newCodexItem.button?.accessibilityIdentifier() == "Usager.StatusItem.codex")
    }
    ...
    func `visibility recovery renders replacement merged status item`() throws {
        ...
        #expect(mergedButton.image != nil)
        #expect(controller.statusItem.autosaveName == "usager-merged")
        #expect(mergedButton.accessibilityIdentifier() == "Usager.StatusItem")
    }
}
