...
struct CLIOutputTests {
    ...
    func `cli error payload is JSON array`() throws {
        let payload = UsagerCLI.makeCLIErrorPayload(
            message: "Nope",
            code: .failure,
            kind: .args,
            pretty: false)
    ...
    func `exit omits generic error when command already emitted payload`() {
        #expect(!UsagerCLI.shouldPrintExitError(code: .success, message: nil))
        #expect(!UsagerCLI.shouldPrintExitError(code: .failure, message: nil))
        #expect(UsagerCLI.shouldPrintExitError(code: .failure, message: "Nope"))
    }
