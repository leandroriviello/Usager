...
struct CLIServeRouterTests {
    ...
    func `health response reports ok status and build version`() throws {
        let response = UsagerCLI.serveHealthResponse(version: "1.2.3")
    ...
    func `health response omits version detail when unavailable`() throws {
        let response = UsagerCLI.serveHealthResponse(version: nil)
        #expect(response.status == .ok)
        let object = try JSONSerialization.jsonObject(with: response.body) as? [String: Any]
        #expect(object?["status"] as? String == "ok")
    ...
    func `serve numeric options reject malformed values`() {
        #expect(UsagerCLI.decodeServePort(from: ParsedValues(
            positional: [],
            options: ["port": ["abc"]],
            flags: [])) == nil)
        #expect(UsagerCLI.decodeServePort(from: ParsedValues(
        ...
        #expect(UsagerCLI.decodeServeRefreshInterval(from: ParsedValues(
            positional: [],
        ...
        #expect(UsagerCLI.decodeServeRequestTimeout(from: ParsedValues(
            positional: [],
    ...
    func `serve help documents request timeout option`() {
        let serve = UsagerCLI.serveHelp(version: "0.0.0")
        let root = UsagerCLI.rootHelp(version: "0.0.0")

        #expect(serve.contains("--request-timeout <seconds>"))
        #expect(serve.contains("usager serve --port 8080 --refresh-interval 60 --request-timeout 30"))
    ...
    func `serve config snapshot reflects provider changes`() throws {
        ...
        var firstConfig = UsagerConfig.makeDefault()
        firstConfig.setProviderConfig(ProviderConfig(id: .opencodego, enabled: false))
        try store.save(firstConfig)

        let firstSnapshot = try UsagerCLI.loadServeConfigSnapshot(configStore: store)
        ...
        let secondSnapshot = try UsagerCLI.loadServeConfigSnapshot(configStore: store)
        ...
        #expect(
            UsagerCLI.serveCacheKey(kind: "usage", provider: nil, configToken: firstSnapshot.cacheToken) !=
                UsagerCLI.serveCacheKey(kind: "usage", provider: nil, configToken: secondSnapshot.cacheToken))
    }
    ...
    func `serve cache skips provider error payloads`() {
        ...
        #expect(UsagerCLI.shouldCacheServeResponse(success))
        #expect(!UsagerCLI.shouldCacheServeResponse(providerError))
        #expect(!UsagerCLI.shouldCacheServeResponse(routeError))
    }

    @Test
    func `serve provider timeout stays below the request deadline`() throws {
        let thirtySecondTimeout = try #require(UsagerCLI.serveProviderTimeout(requestTimeout: 30))
        let tenSecondTimeout = try #require(UsagerCLI.serveProviderTimeout(requestTimeout: 10))
        ...
        #expect(UsagerCLI.serveProviderTimeout(requestTimeout: 0) == nil)
        #expect(UsagerCLI.serveProviderTimeout(requestTimeout: .infinity) == nil)
        // Finite deadlines stay strictly below the request timeout at every
        // value, including sub-second ones.
        let oneSecondTimeout = try #require(UsagerCLI.serveProviderTimeout(requestTimeout: 1))
        let halfSecondTimeout = try #require(UsagerCLI.serveProviderTimeout(requestTimeout: 0.5))
        ...
        let oversizedTimeout = try #require(UsagerCLI.serveProviderTimeout(
    ...
    func `serve usage collection bounds a hung provider without blocking others`() async {
        ...
        let start = Date()
        let output = await UsagerCLI.serveCollectUsageOutputs(
            providers: providers,
            providerTimeout: 0.1)
        { provider in
            if provider == .claude {
    ...
    func `serve cache uses stable Codex account identities`() {
        ...
        let firstKey = UsagerCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: firstProjection)
        let reshapedKey = UsagerCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: reshapedProjection)
        let replacementKey = UsagerCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: replacement)
        let workspacePeerKey = UsagerCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: workspacePeer)
        let storedBeforeKey = UsagerCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: storedBeforeRefresh)
        let storedAfterKey = UsagerCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
        ...
        #expect(UsagerCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
    ...
    func `serve cache coalesces concurrent cache misses`() async {
        ...
        let responses = await withTaskGroup(of: CLILocalHTTPResponse.self) { group -> [CLILocalHTTPResponse] in
            for _ in 0..<5 {
                group.addTask {
                    await UsagerCLI.cachedServeResponse(
                        key: "usage:",
                        cache: cache,
                        refreshInterval: 60,
                        requestTimeout: 1)
    ...
    func `serve cache prunes expired config token entries`() async throws {
        ...
        _ = await UsagerCLI.cachedServeResponse(
    ...
    func `serve cache does not cache timeouts and recovers on next success`() async {
        ...
        let timeout = await UsagerCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 0.01)
        ...
        let success = await UsagerCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 1)
        ...
        let cached = await UsagerCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 1)
    ...
    func `serve cache serves last good payload when refresh fails`() async {
        ...
        let first = await UsagerCLI.cachedServeResponse(
        ...
        let failed = await UsagerCLI.cachedServeResponse(
            key: "usage:antigravity",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
        ...
        let recovered = await UsagerCLI.cachedServeResponse(
            key: "usage:antigravity",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
    ...
    func `serve cache replaces only failed provider account rows`() async throws {
        ...
        let refreshed = await UsagerCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
    ...
    func `serve cache fails closed on timeout after merged rows`() async {
        ...
        let timedOut = await UsagerCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 0.01)
    ...
    func `serve stale ttl is bounded and disabled without caching`() {
        #expect(UsagerCLI.serveStaleTTL(refreshInterval: 0) == 0)
        #expect(UsagerCLI.serveStaleTTL(refreshInterval: 1) == 300)
        #expect(UsagerCLI.serveStaleTTL(refreshInterval: 60) == 600)
        #expect(UsagerCLI.serveStaleTTL(refreshInterval: 1800) == 3600)
        #expect(UsagerCLI.serveStaleTTL(refreshInterval: 86401) == 3600)
        #expect(UsagerCLI.serveStaleTTL(refreshInterval: .infinity) == 3600)
    }
    ...
    func `serve helper idle window outlives the refresh cadence`() {
        #expect(UsagerCLI.serveCLISessionIdleWindow(refreshInterval: 0) == 180)
        #expect(UsagerCLI.serveCLISessionIdleWindow(refreshInterval: 60) == 180)
        #expect(UsagerCLI.serveCLISessionIdleWindow(refreshInterval: 300) == 360)
    }
    ...
    func `serve request timeout zero disables the deadline`() async {
        ...
        let response = await UsagerCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0,
            requestTimeout: 0)
