...
struct CLIDiagnoseCommandTests {
    ...
    func `diagnose help describes generic JSON export`() {
        let help = UsagerCLI.diagnoseHelp(version: "0.0.0")

        #expect(help.contains("usager diagnose --provider <name|all> --format json"))
        #expect(help.contains("usager diagnose --provider all --format json"))
    ...
    func `diagnose output writer creates parent directories`() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("UsagerDiagnoseTests-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: root) }

        let output = root.appendingPathComponent("nested/diagnostic.json")
        try UsagerCLI.writeDiagnosticExport(#"{"provider":"minimax"}"#, to: output.path)
    ...
    func `diagnose auth mode uses settings-backed MiniMax manual cookie when env token is absent`() {
        ...
        let authMode = UsagerCLI._resolveMiniMaxAuthModeForTesting(
            environment: [:],
            settings: settings)
    ...
    func `generic diagnose auth summary detects provider config`() {
        let summary = UsagerCLI._diagnosticAuthSummaryForTesting(
            provider: .openai,
            account: nil,
    ...
    func `generic diagnose auth summary requires complete Bedrock credentials`() {
        let partial = UsagerCLI._diagnosticAuthSummaryForTesting(
            provider: .bedrock,
            account: nil,
        ...
        let complete = UsagerCLI._diagnosticAuthSummaryForTesting(
            provider: .bedrock,
            account: nil,
            config: nil,
            environment: [
