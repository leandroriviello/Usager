...
struct CLIProviderSelectionTests {
    ...
    func `help includes gemini and all`() {
        let usage = UsagerCLI.usageHelp(version: "0.0.0")
        ...
        #expect(usage.contains("usager usage --provider gemini"))
        #expect(usage.contains("usager usage --format json --provider all --pretty"))
        #expect(root.contains("usager --provider gemini"))
    }
    ...
    func `provider selection respects override`() {
        let selection = UsagerCLI.providerSelection(rawOverride: "gemini", enabled: [.codex, .claude])
        #expect(selection.asList == [.gemini])
    }

    @Test
    func `provider selection uses enabled providers when three or more are enabled`() {
        let selection = UsagerCLI.providerSelection(
            rawOverride: nil,
    ...
    func `provider selection does not expand three enabled providers to all providers`() {
        ...
        let selection = UsagerCLI.providerSelection(rawOverride: nil, enabled: enabled)
    ...
    func `provider selection accepts kiro alias`() {
        let selection = UsagerCLI.providerSelection(rawOverride: "kiro-cli", enabled: [.codex])
