...
struct CLICostTests {
    ...
    func `cost json shortcut does not enable json logs`() throws {
        let signature = UsagerCLI._costSignatureForTesting()
    ...
    func `renders cost text snapshot`() {
        ...
        let output = UsagerCLI.renderCostText(provider: .claude, snapshot: snap, useColor: false)
    ...
    func `renders codex project grouped cost text`() {
        ...
        let output = UsagerCLI.renderCostText(
            provider: .codex,
            snapshot: snap,
            groupBy: .project,
            useColor: false)
    ...
    func `codex cost payload includes project rollups`() throws {
        let snapshot = CostUsageTokenSnapshot(
            ...
            updatedAt: Date(timeIntervalSince1970: 1_700_000_000))
        let payload = UsagerCLI.makeCostPayload(provider: .codex, snapshot: snapshot, error: nil)
