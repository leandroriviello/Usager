...
struct CodexLoginRunnerTests {
    ...
    func `login runner returns timeout before hung codex exits`() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("usager-login-runner-\(UUID().uuidString)", isDirectory: true)
    ...
    func `login runner bounds output drain when detached child keeps pipes open`() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("usager-login-drain-\(UUID().uuidString)", isDirectory: true)
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let homeDir = root.appendingPathComponent("home", isDirectory: true)
        ...
        child_pid=$!
        printf '%s\\n' "$child_pid" > "$USAGER_TEST_CHILD_PID_FILE"
        ...
        let result = await CodexLoginRunner.run(
            ...
            environment: [
                "USAGER_TEST_CHILD_PID_FILE": childPIDFile.path,
                "PATH": binDir.path,
            ],
            loginPATH: nil)
        let elapsed = Date().timeIntervalSince(start)
