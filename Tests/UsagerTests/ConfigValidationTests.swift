...
struct ConfigValidationTests {
    ...
    func `fresh config defaults Alibaba Token Plan to International`() throws {
        ...
        let issues = UsagerConfigValidator.validate(config)
    ...
    func `config store default url honors environment override`() {
        let url = UsagerConfigStore.defaultURL(environment: [
            UsagerConfigStore.pathEnvironmentKey: "~/tmp/usager-test-config.json",
        ])

        #expect(url.path.hasSuffix("/tmp/usager-test-config.json"))
    }
    ...
    func `config store default url honors xdg config home`() throws {
        ...
        let url = UsagerConfigStore.defaultURL(
            home: home,
            environment: [
                UsagerConfigStore.xdgConfigHomeEnvironmentKey: xdgHome.path,
            ],
    ...
    func `config store default url ignores relative xdg config home`() throws {
        ...
        let url = UsagerConfigStore.defaultURL(
            home: home,
            environment: [
                UsagerConfigStore.xdgConfigHomeEnvironmentKey: "relative-config",
            ],
            fileManager: fileManager)
    ...
    func `config store default url creates in xdg default for new installs`() throws {
        ...
        let url = UsagerConfigStore.defaultURL(home: home, environment: [:], fileManager: fileManager)
    ...
    private static func makeTemporaryHome() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("UsagerConfigStoreTests-\(UUID().uuidString)", isDirectory: true)
