---
name: release-usager
description: "Usager release: versioning, notarization, appcast, Homebrew, post-release bump."
---
...
- repo: `~/Projects/usager`
- release script: `Scripts/release.sh`
- signing/notarization: `Scripts/sign-and-notarize.sh`
- appcast: `Scripts/make_appcast.sh`, `appcast.xml`
- release assets: `Usager-macos-universal-<version>.zip`, `Usager-macos-universal-<version>.dSYM.zip`
- packaged app: `Usager.app`
- version file: `version.env`
- changelog: `CHANGELOG.md`
- Homebrew tap: `~/Projects/homebrew-tap`
- cask: `~/Projects/homebrew-tap/Casks/usager.rb`
- formula: `~/Projects/homebrew-tap/Formula/usager.rb`
- CLI release workflow: `.github/workflows/release-cli.yml`

Normal release:

```bash
tmux new-session -d -s usager-release 'op run --account my.1password.com --env-file /tmp/usager-release-op.env -- Scripts/release.sh'
tmux attach -t usager-release
```
...
If widget metadata generation times out, `USAGER_WIDGET_METADATA_TIMEOUT_SECONDS=600` is a known-good floor.

Usager CLI tarballs are not produced by `Scripts/release.sh` itself. The GitHub release event triggers `.github/workflows/release-cli.yml`, which builds and uploads:

- `UsagerCLI-v<version>-macos-arm64.tar.gz`
- `UsagerCLI-v<version>-macos-x86_64.tar.gz`
- `UsagerCLI-v<version>-linux-aarch64.tar.gz`
- `UsagerCLI-v<version>-linux-x86_64.tar.gz`
- matching `.sha256` files

If the workflow fails only in `update-homebrew-tap` with GitHub API rate limiting, the CLI assets may already be uploaded. Verify assets live, then update `Formula/usager.rb` manually from the tarball checksums.
...
PY
codesign --verify --deep --strict --verbose=2 Usager.app
spctl --assess --type execute --verbose Usager.app
```

For Homebrew:

```bash
shasum -a 256 Usager-macos-universal-<VERSION>.zip
cd /Users/steipete/Projects/homebrew-tap
python3 .github/scripts/update_formula.py --formula usager --tag v<VERSION> --repository steipete/Usager --artifact-template 'UsagerCLI-{tag}-{target}.tar.gz' --target-aliases 'darwin_arm64=macos-arm64,darwin_amd64=macos-x86_64,linux_arm64=linux-aarch64,linux_amd64=linux-x86_64'
brew fetch --cask --force --retry usager
brew fetch --formula --force --retry steipete/tap/usager
```
...
open -n Usager.app
/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' Usager.app/Contents/Info.plist
/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' Usager.app/Contents/Info.plist
```
