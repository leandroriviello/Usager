...
SCRIPT = Path(__file__).with_name("usager")
MODULE = runpy.run_path(str(SCRIPT), run_name="usager_skill")
...
def make_env(root: Path, *, install: bool = True) -> tuple[dict[str, str], Path]:
    binary = root / "Usager.app" / "Contents" / "Helpers" / "UsagerCLI"
    binary.parent.mkdir(parents=True)
    binary.write_text(FAKE, encoding="utf-8")
    binary.chmod(0o755)
    env = os.environ.copy()
    env["USAGER_SKIP_DISCOVERY"] = "1"
    env["USAGER_TIMEOUT"] = "3"
    env["FAKE_LOG"] = str(root / "argv.log")
    env["FAKE_VERSION_STDOUT"] = "Usager 1.2.3\n"
    ...
    if install:
        env["USAGER_BIN"] = str(binary)
    else:
        env.pop("USAGER_BIN", None)
...
class UsagerSkillTests(unittest.TestCase):
    def test_help_is_small_and_read_only(self) -> None:
        result = helper("--help", env=os.environ.copy())
        self.assertEqual(result.returncode, 0)
        self.assertIn("Usager read. JSON out. No writes.", result.stdout)
    ...
    def test_doctor_reports_version_and_raw_validation_shape(self) -> None:
        ...
        self.assertEqual(payload["version"], "Usager 1.2.3")
