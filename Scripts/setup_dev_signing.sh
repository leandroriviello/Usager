...
# Check if we already have a Usager development certificate
CERT_NAME="Usager Development"
...
openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 \
    -nodes -keyout /tmp/usager-dev.key -out /tmp/usager-dev.crt \
    -config "$TEMP_CONFIG" 2>/dev/null

# Convert to PKCS12 format
openssl pkcs12 -export -out /tmp/usager-dev.p12 \
    -inkey /tmp/usager-dev.key -in /tmp/usager-dev.crt \
    -passout pass: 2>/dev/null

# Import into keychain
security import /tmp/usager-dev.p12 -k ~/Library/Keychains/login.keychain-db -T /usr/bin/codesign -T /usr/bin/security

# Clean up temporary files
rm -f /tmp/usager-dev.{key,crt,p12}
