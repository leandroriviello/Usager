...
Usager imports your browser session cookie automatically. Usager sends the cookie only to
`https://t3.chat`.
...
5. Paste the full `curl` command into the **T3 Chat cookie** field in Usager settings.
...
```bash
# Show T3 Chat usage
usager usage --provider t3chat

# Or use the alias
usager usage --provider t3-chat
usager usage --provider t3
```

T3 Chat provides no token-cost data. The `usage --format json` output contains usage and identity
data, while `usager cost --provider t3chat` is unsupported.
...
| `T3 Chat session cookie is invalid or expired` | Stale or revoked session cookie | Sign out of T3 Chat, sign back in, then refresh Usager or repaste the cookie |
...
| `Could not parse T3 Chat usage` | T3 Chat changed its tRPC response shape | Open a Usager issue with a redacted response sample |
...
- `Sources/UsagerCore/Providers/T3Chat/T3ChatProviderDescriptor.swift` — provider metadata and fetch pipeline
- `Sources/UsagerCore/Providers/T3Chat/T3ChatUsageFetcher.swift` — tRPC request, cookie import, and cURL parsing
- `Sources/UsagerCore/Providers/T3Chat/T3ChatUsageSnapshot.swift` — response decoding and window mapping
- `Sources/Usager/Providers/T3Chat/T3ChatProviderImplementation.swift` — settings pickers and bindings
- `Sources/Usager/Providers/T3Chat/T3ChatSettingsStore.swift` — cookie source and header persistence
