...
Crof is API-only. Usager reads `GET https://crof.ai/usage_api/` with a
...
1. **API key** supplied via `CROF_API_KEY`, `CROFAI_API_KEY`, or Settings →
   Providers → Crof. Settings values are stored in `~/.usager/config.json`.
2. **Usage endpoint**
...
- Crof support said quota reset is around midnight Central time; Usager models this
...
- The provider icon is SVG and Usager renders it as a template image so it
  matches the other monochrome provider icons.
- Dashboard: `https://crof.ai/dashboard`.

## Related files

- `Sources/UsagerCore/Providers/Crof/`
- `Sources/Usager/Providers/Crof/`
- `Tests/UsagerTests/CrofUsageFetcherTests.swift`
