...
Track live Codex + Claude Code agent sessions — local Mac first, other Macs on the tailnet second — and surface them in the Usager menu with click-to-focus of the owning terminal window.

## Why in Usager

Usager already parses `~/.claude/projects` JSONL (cost scanner) and ships a bundled CLI on macOS + Linux. Sessions reuse both: the local scanner feeds the menu UI, and the same scanner exposed as `usager sessions --json` is what remote Macs run over SSH. No daemon, no new app.

## Data model (UsagerCore)
...
## Local scanner (UsagerCore, no new deps)
...
- `usager sessions` — table; `--json` — `[AgentSession]` (stable field names above; ISO-8601 dates).
- `usager sessions focus <id>` — macOS only: focus the session's terminal window (see Focus). Exit 1 if id unknown, 2 if focus failed.
- Follows existing `CLI*Command.swift` conventions. Works on Linux for listing (ps/proc paths guarded), focus is Darwin-only.

## Remote hosts (UsagerCore + app)
...
- Fetch per host (parallel, 5 s budget): `ssh -o BatchMode=yes -o ConnectTimeout=3 <host> sh -lc 'usager sessions --json'` with fallback to the bundled app CLI path (resolve the canonical bundled location from `Scripts/package_app.sh` and hardcode it as fallback: `… || <bundled-path> sessions --json`). Host errors are non-fatal: host shown as unreachable, others still render.
- Remote focus: fire-and-forget `ssh <host> sh -lc 'usager sessions focus <id>'`.
...
`make check` clean; `make test` (or focused `swift test --filter` covering the new tests) green; `swift run UsagerCLI sessions --json` produces plausible output on this Mac.
