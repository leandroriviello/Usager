...
1) **Cached cookie header** (Keychain cache `com.leandroriviello.usager.cache`, account `cookie.factory`).
2) **Stored session** (`~/Library/Application Support/Usager/factory-session.json`).
...
## Session storage
- File: `~/Library/Application Support/Usager/factory-session.json`
...
## Key files
- `Sources/UsagerCore/Providers/Factory/FactoryStatusProbe.swift`
- `Sources/UsagerCore/Providers/Factory/FactoryLocalStorageImporter.swift`
