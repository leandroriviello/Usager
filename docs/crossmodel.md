...
[CrossModel](https://crossmodel.ai) is a multi-provider, OpenAI- and Anthropic-compatible API aggregation platform. You call one API and CrossModel routes the request to the right upstream provider, billing a prepaid USD wallet.
...
1. **Credits API** (`/v1/credits`): Returns the wallet balance (`balance_micro`) and any in-flight holds (`uncollected_micro`). All amounts are integer micro units (1 USD = 1,000,000 micro).

2. **Usage API** (`/v1/usage`): Returns spend, token, and request counts for the current UTC day, ISO week, and calendar month.

## Display

The CrossModel menu card shows:

- **Balance**: Displayed in the identity section as "Balance: $X.XX" and (optionally) in the menu bar
...
- CrossModel uses a prepaid USD wallet; there is no per-key spending limit, so no quota meter is shown.
- The balance call is required; the usage call is best-effort and will not block the balance if it is slow or unavailable.
