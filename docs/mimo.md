...
Usager imports cookies from these browsers in order: **Safari**, **Chrome** / **Chrome Beta** / **Chrome Canary**, **Firefox**, and **Microsoft Edge**. Switch to **Manual** and paste a `Cookie:` header if your active MiMo session lives in Arc, Brave, or another browser profile Usager does not auto-detect.

Safari cookie import may require granting Usager Full Disk Access in **System Settings → Privacy & Security**.
...
- Supports `MIMO_API_URL` to override the base API URL for testing. Override values must be explicit HTTPS URLs or
  bare hosts/paths that Usager normalizes to HTTPS. Explicit `http://` values fail closed before MiMo cookies are
...
Log in at `https://platform.xiaomimimo.com/#/console/balance` in Safari, Chrome, Firefox, or Edge, then refresh Usager. If your session lives in another browser, switch the MiMo provider to **Cookie source → Manual** and paste the `Cookie:` header instead.
...
2. Run `mimo-usage --update` once to populate `~/.usager/mimo-local-usage.json`. The tracker scans `~/.claude-envs/mimo/.claude/projects/**/*.jsonl` (default path for a `cc-mimo`-style wrapper) and aggregates input, output, cache-read, and cache-creation tokens per time window (today / this week / all time).
...
- Cache schema (`~/.usager/mimo-local-usage.json`) is internal; do not rely on the JSON shape for external tooling.
