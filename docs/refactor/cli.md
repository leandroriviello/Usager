...
read_when:
  - "Refactoring Usager CLI error handling or config parsing."
...
- (Optional) `usager config dump`
  - Prints normalized config JSON.

## Step-by-step implementation guide
1. **Add validation types**
   - `UsagerConfigIssue` + `UsagerConfigValidator` in `UsagerCore/Config`.
   - Keep file <500 LOC.
...
8. **Verification**
   ...
   - CLI e2e: `usager --json-only ...`, `usager config validate`.
