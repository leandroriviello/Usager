...
> Replicate Codex's direct OAuth token usage in Usager instead of calling the CLI.

## Background

Currently, Usager fetches Codex usage by:
...
- Command: `UsagerCLI usage --provider codex --source oauth --json --pretty`
...
|------|----------|---------|
| `CodexOAuthCredentials.swift` | `Sources/UsagerCore/Providers/Codex/CodexOAuth/` | Token storage model + loader |
| `CodexOAuthUsageFetcher.swift` | `Sources/UsagerCore/Providers/Codex/CodexOAuth/` | API client for usage endpoint |
| `CodexTokenRefresher.swift` | `Sources/UsagerCore/Providers/Codex/CodexOAuth/` | Token refresh logic |
...
public enum CodexOAuthUsageFetcher {
    ...
    public static func fetchUsage(
        ...
        request.setValue("Usager", forHTTPHeaderField: "User-Agent")
...
2. Run Usager with debug logging enabled
