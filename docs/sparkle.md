---
summary: "Sparkle integration details for Usager: updater config, keys, and release flow."
read_when:
...
- Updater: `SPUStandardUpdaterController` owned by `AppDelegate` (see `Sources/Usager/CodexbarApp.swift:1`).
...
## Release flow
1) Build & notarize as usual (`./Scripts/sign-and-notarize.sh`), producing notarized `Usager-macos-universal-<ver>.zip`.
...
- Usager disables Sparkle in Homebrew and unsigned builds; those installs should be updated via `brew` or reinstalling from Releases.
