...
read_when:
  ...
  - Troubleshooting LiteLLM API-key usage in Usager
---
...
Configure it in Settings -> Providers -> LiteLLM, or in `~/.usager/config.json`:
...
`LITELLM_BASE_URL` may include `/v1`; Usager strips that suffix before calling LiteLLM management endpoints.
...
All requests use `Authorization: Bearer <apiKey>`. Usager does not request or store a LiteLLM master key.
...
endpoint. Usager validates returned user and team IDs against `/key/info` before displaying usage.

## Security

Treat LiteLLM keys as secrets. Usager stores configured keys only in provider config or token-account storage and
