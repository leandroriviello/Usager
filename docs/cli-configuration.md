---
summary: "Usager CLI configuration commands for provider toggles, API keys, and isolated config files."
read_when:
  - Using usager config from scripts or CI
...
`usager config` edits the same resolved config file used by the app's Settings → Providers pane.
New installs use `~/.config/usager/config.json`; absolute `XDG_CONFIG_HOME` paths and `USAGER_CONFIG` are
supported, and existing `~/.usager/config.json` installs keep using the legacy file when no XDG config exists.
...
usager config providers --json --pretty
```
...
These are persistent app/CLI settings. They are different from `usager usage --provider grok`, which is a one-shot
command override and does not edit config.

If every provider is disabled, `usager usage` with no `--provider` prints no text output, and
`usager usage --json` prints `[]`. Passing `--provider <name>` still fetches that provider for the one command.
...
```bash
printf '%s' "$OPENROUTER_API_KEY" | usager config set-api-key --provider openrouter --stdin --no-enable
```
...
printf '%s' "$ANTHROPIC_ADMIN_KEY" | usager config set-api-key --provider claude --stdin
printf '%s' "$DEEPGRAM_API_KEY" | usager config set-api-key --provider deepgram --stdin
printf '%s' "$GROQ_API_KEY" | usager config set-api-key --provider groq --stdin
printf '%s' "$LLM_PROXY_API_KEY" | usager config set-api-key --provider llmproxy --stdin
...
sessions instead of an xAI API key for Usager's billing view, so enable them with
`usager config enable --provider grok`.

LLM Proxy also needs a base URL. Use `LLM_PROXY_BASE_URL` for CLI runs, or add `"enterpriseHost"` to the provider entry
in the Usager config file.

## Isolated config files

For tests, demos, and CI, point Usager at a temporary config file:

```bash
export USAGER_CONFIG=/tmp/usager-config.json
usager config enable --provider grok
usager config providers --json --pretty
```
...
```bash
usager cost --provider codex --days 90
usager cost --provider claude --days 180 --format json --pretty
```
