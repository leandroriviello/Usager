...
- `Sources/UsagerCore`: provider descriptors + fetch strategies + probes + parsing + shared utilities.
- `Sources/Usager`: UI/state + provider implementations (settings/login/menu hooks only).
...
- `Sources/UsagerCore/Providers/<ProviderID>/`
...
- `Sources/Usager/Providers/<ProviderID>/`
...
Checklist:
- Add `UsageProvider` case in `Sources/UsagerCore/Providers/Providers.swift`.
- Create `Sources/UsagerCore/Providers/<ProviderID>/`:
...
- Add `Sources/Usager/Providers/<ProviderID>/<ProviderID>ProviderImplementation.swift`:
