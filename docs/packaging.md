...
## Scripts
- `Scripts/package_app.sh`: builds host arch with ad-hoc signing by default; set `ARCHES="arm64 x86_64"` for universal. Verifies slices. Stable-certificate packaging requires explicit `USAGER_SIGNING=identity` plus `APP_IDENTITY`.
...
## Bundle contents
- `UsagerWidget.appex` is built by `WidgetExtension/UsagerWidgetExtension.xcodeproj` as a real macOS app extension, then bundled with app-group entitlements.
- `UsagerCLI` copied to `Usager.app/Contents/Helpers/` for symlinking.
