...
extension UsagerCLI {
    ...
    static func runServe(_ values: ParsedValues) async {
        ...
        do {
            try await server.run {
                Self.writeStderr("Usager server listening on http://127.0.0.1:\(port)\n")
            }
        } catch {
    ...
    static func loadServeConfigSnapshot(
        configStore: UsagerConfigStore = UsagerConfigStore()) throws -> CLIServeConfigSnapshot
    {
        let config = try configStore.load() ?? UsagerConfig.makeDefault()
    ...
    static func serveConfigCacheToken(for config: UsagerConfig) throws -> String {
    ...
    static func cachedServeResponse(
        ...
        requestTimeout: TimeInterval = UsagerCLI.defaultServeRequestTimeout,
    ...
    private static func serveCost(provider rawProvider: String?, config: UsagerConfig) async -> CLILocalHTTPResponse {
        let selection: ProviderSelection
        do {
            selection = try Self.serveProviderSelection(rawProvider: rawProvider, config: config)
        } catch {
    ...
    private static func serveProviderSelection(
        rawProvider: String?,
        config: UsagerConfig) throws -> ProviderSelection
    {
