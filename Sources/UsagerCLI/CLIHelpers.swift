...
extension UsagerCLI {
    static func decodeProvider(from values: ParsedValues, config: UsagerConfig) -> ProviderSelection {
    ...
    static func resetTimeDisplayStyleFromDefaults() -> ResetTimeDisplayStyle {
        let domains = [
            "com.leandroriviello.usager",
            "com.leandroriviello.usager.debug",
        ]
    ...
    static func loadConfig(output: CLIOutputPreferences) -> UsagerConfig {
        let store = UsagerConfigStore()
        do {
            if let existing = try store.load() {
                return existing
            }
            return UsagerConfig.makeDefault()
        } catch {
