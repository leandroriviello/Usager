...
enum UsagerCLI {
    ...
    private static func bootstrapLogging(path: [String], values: ParsedValues) {
        UsagerLog.bootstrapIfNeeded(self.loggingConfiguration(path: path, values: values))
    }

    static func loggingConfiguration(path: [String], values: ParsedValues) -> UsagerLog.Configuration {
        ...
        let destination: UsagerLog.Destination = path == ["diagnose"] ? .discard : .stderr
        return .init(destination: destination, level: level, json: isJSON)
    }

    static func resolvedLogLevel(verbose: Bool, rawLevel: String?) -> UsagerLog.Level {
        UsagerLog.parseLevel(rawLevel) ?? (verbose ? .debug : .error)
    }
