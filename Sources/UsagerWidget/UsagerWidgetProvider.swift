...
struct UsagerWidgetEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
    let snapshot: WidgetSnapshot
}

struct UsagerCompactEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
...
struct UsagerSwitcherEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
...
struct UsagerTimelineProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> UsagerWidgetEntry {
        UsagerWidgetEntry(
            date: Date(),
            provider: .codex,
    ...
    func snapshot(for configuration: ProviderSelectionIntent, in context: Context) async -> UsagerWidgetEntry {
        let provider = configuration.provider.provider
        return UsagerWidgetEntry(
            date: Date(),
            provider: provider,
...
struct UsagerSwitcherTimelineProvider: TimelineProvider {
    ...
    func getSnapshot(in context: Context, completion: @escaping (UsagerSwitcherEntry) -> Void) {
        completion(self.makeEntry())
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<UsagerSwitcherEntry>) -> Void) {
    ...
    private func makeEntry() -> UsagerSwitcherEntry {
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.emptySnapshot()
        let providers = self.availableProviders(from: snapshot)
...
struct UsagerCompactTimelineProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> UsagerCompactEntry {
        UsagerCompactEntry(
            date: Date(),
            provider: .codex,
            metric: .credits,
    ...
    func snapshot(for configuration: CompactMetricSelectionIntent, in context: Context) async -> UsagerCompactEntry {
        let provider = configuration.provider.provider
        let metric = configuration.metric
        return UsagerCompactEntry(
            date: Date(),
            provider: provider,
            metric: metric,
            snapshot: WidgetSnapshotStore.load() ?? WidgetPreviewData.snapshot())
    ...
    func timeline(
        ...
        in context: Context) async -> Timeline<UsagerCompactEntry>
    {
        let provider = configuration.provider.provider
        let metric = configuration.metric
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.emptySnapshot()
        let entry = UsagerCompactEntry(
            date: Date(),
            provider: provider,
            metric: metric,
            snapshot: snapshot)
