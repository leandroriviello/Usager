...
/// process. ``agy`` is an interactive REPL, not a query command, so Usager
...
struct AntigravityCLIHTTPSFetchStrategy: ProviderFetchStrategy {
    ...
    private static let log = UsagerLog.logger(LogCategories.antigravity)
    ...
    struct WarmAgyDependencies {
        ...
        /// The pid of an ``agy`` that Usager itself spawned and manages through
        ...
        /// long-lived `agy`, or another Usager host) has no such accounting.
        ...
        dependencies: WarmAgyDependencies) async throws -> AntigravityStatusSnapshot?
    {
        ...
        // Also exclude any `agy` Usager itself spawned and manages: reusing it
    ...
    static func liveWarmAgyDependencies() -> WarmAgyDependencies {
        WarmAgyDependencies(
            ...
            ownedPID: {
                // The pid of the `agy` Usager manages through the shared
        ...
        async throws -> ProviderFetchResult
    {
        ...
        // process (an IDE, a long-lived `agy`, or another Usager host), so
        // Usager must not manage its lifecycle, idle timeout, or
        // `resetAfterFetch` teardown. Those apply only to processes Usager
    ...
    /// Spawn (or reuse Usager's own warm) `agy` session and wait for the CLI
    ...
    static func shouldResetSessionAfterFetch(_ context: ProviderFetchContext) -> Bool {
        // Long-lived hosts (the app, `usager serve`) keep the warm `agy`
