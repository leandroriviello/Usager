...
actor ClaudeCLISession {
    ...
    private static let log = UsagerLog.logger(LogCategories.claudeCLI)
    #if DEBUG
    ...
    private func ensureStarted(binary: String) throws {
        ...
        let disableWatchdog = ProcessInfo.processInfo.environment["USAGER_DISABLE_CLAUDE_WATCHDOG"] == "1"
        if !disableWatchdog,
           resolvedURL.lastPathComponent == "claude",
           let watchdog = TTYCommandRunner.locateBundledHelper("UsagerClaudeWatchdog")
        {
