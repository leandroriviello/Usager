...
public enum ClaudeOAuthCredentialsStore {
    ...
    public static let environmentTokenKey = "USAGER_CLAUDE_OAUTH_TOKEN"
    public static let environmentScopesKey = "USAGER_CLAUDE_OAUTH_SCOPES"
    ...
    public static let environmentClientIDKey = "USAGER_CLAUDE_OAUTH_CLIENT_ID"
    ...
    static let log = UsagerLog.logger(LogCategories.claudeUsage)
    ...
    private struct Repository {
        ...
        private func resolvedCacheOwner(_ owner: ClaudeOAuthCredentialOwner) -> ClaudeOAuthCredentialOwner {
            guard owner == .usager else { return owner }
        ...
        func shouldAttemptFreshnessSyncFromClaudeKeychain(cached: ClaudeOAuthCredentialRecord) -> Bool {
            ...
            case .onlyOnUserAction:
                if ProviderInteractionContext.current != .userInitiated {
                    if ProcessInfo.processInfo.environment["USAGER_DEBUG_CLAUDE_OAUTH_FLOW"] == "1" {
                        ClaudeOAuthCredentialsStore.log.debug(
    ...
    /// - Usager-owned credentials refresh directly via token endpoint.
    ...
    /// Save refreshed credentials to Usager's keychain cache
        ...
        historyOwnerIdentifier: String?)
    {
        ...
        self.saveToCacheKeychain(
            ...
            owner: .usager,
            historyOwnerIdentifier: historyOwnerIdentifier)
        self.log.debug("Saved refreshed credentials to Usager keychain cache")
    }
    ...
    /// Check if Usager has cached credentials (in memory or keychain cache)
...
extension ClaudeOAuthCredentialsStore {
    ...
    /// Usager's caches. This is used to avoid triggering a second OS keychain dialog during the OAuth retry.
    @discardableResult
    ...
    /// Updates Usager's keychain cache with the new credentials.
