...
public enum ClaudeSwapSwitchParserError: LocalizedError, Equatable, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case let .unsupportedSchemaVersion(version):
            "claude-swap switch output uses unsupported schema version \(version); Usager supports version 1."
        ...
        case let .mismatchedTarget(expected, actual):
            "claude-swap reported account slot \(actual) after Usager requested slot \(expected)."
        }
    }
}
