...
struct CursorAppAuthStore: CursorAppAuthSessionProviding {
        ...
        fileManager: FileManager = .default) -> String
    {
        ...
        #elseif os(Linux)
        let configHome = environment[UsagerConfigStore.xdgConfigHomeEnvironmentKey]?
...
public enum CursorStatusProbeError: LocalizedError, Sendable {
    ...
    static let safariFullDiskAccessHint =
        "If you use Safari, grant Usager Full Disk Access in System Settings ▸ Privacy & Security."

    public var errorDescription: String? {
        switch self {
        case .notLoggedIn:
            #if os(macOS)
            "Not logged in to Cursor. Please log in via the Usager menu."
            #else
            "Not logged in to Cursor. Sign in to the Cursor app on this machine or paste a Cookie header copied "
                + "from cursor.com into ~/.config/usager/config.json (legacy: ~/.usager/config.json)."
            #endif
        ...
        case .noSessionCookie:
            ...
            "No Cursor session found. \(Self.safariFullDiskAccessHint) "
                ...
                + "You can also sign in to Cursor from the Usager menu (Add / switch account)."
            #else
...
public actor CursorSessionStore {
    ...
    private init() {
        ...
        let dir = appSupport.appendingPathComponent("Usager", isDirectory: true)
