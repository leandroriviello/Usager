...
final class OpenAIDashboardWebViewCache {
    ...
    fileprivate static let log = UsagerLog.logger(LogCategories.openAIWebview)
    ...
    (() => {
      try {
        delete window.__usagerDidScrollToCredits;
        delete window.__usagerUsageBreakdownJSON;
        delete window.__usagerUsageBreakdownDebug;
        return true;
      } catch {
        return false;
      }
