...
public struct TTYCommandRunner {
    private static let log = UsagerLog.logger(LogCategories.ttyRunner)
    ...
    static func locateBundledHelper(_ name: String) -> String? {
        ...
        if let override = ProcessInfo.processInfo.environment["USAGER_HELPER_\(name.uppercased())"],
        ...
        onURLDetected: (@Sendable () -> Void)? = nil) throws -> Result
    {
        ...
        if isClaudeCLI,
           let watchdog = Self.locateBundledHelper("UsagerClaudeWatchdog")
        {
            executable = watchdog
