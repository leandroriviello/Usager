...
public enum KeychainCacheStore {
    ...
    private static let log = UsagerLog.logger(LogCategories.keychainCache)
    private static let cacheService = "com.leandroriviello.usager.cache"
    private static let cacheLabel = "Usager Cache"
        ...
        fileExists: (String) -> Bool = { FileManager.default.fileExists(atPath: $0) }) -> [String]
    {
        ...
        if let appBundle {
            ...
            append(appBundle.appendingPathComponent("Contents/Helpers/UsagerCLI").path)
        }
