...
final class ProviderSwitcherView: NSView {
    ...
    private let selectedBackground = UsagerBrand.signalNS.withAlphaComponent(0.14).cgColor
    private let unselectedBackground = NSColor.clear.cgColor
    private let selectedTextColor = UsagerBrand.signalNS
    private let unselectedTextColor = UsagerBrand.secondaryNS
    private let stackedIcons: Bool
    private let rowCount: Int
    private let rowSpacing: CGFloat
    private let rowHeight: CGFloat
    private let overviewPresentation: Bool
        ...
        onSettings: (() -> Void)? = nil)
    {
        ...
        self.overviewPresentation = selected == .overview
        ...
        let height: CGFloat = self.overviewPresentation
            ? 42
            : self.rowHeight * CGFloat(self.rowCount)
        ...
        if self.overviewPresentation {
            self.installOverviewHeader()
        }
        if width > 0 {
            self.preferredWidth = width
    ...
    private func installOverviewHeader() {
...
extension ProviderSwitcherView {
        ...
        remainingPercent _: Double) -> NSColor
    {
        ...
        case let .provider(provider):
            _ = provider
            return UsagerBrand.signalNS
        case .overview:
            return UsagerBrand.secondaryNS
        }
    }
