...
extension SettingsStore {
    private func updateConfig(reason: String, mutate: (inout UsagerConfig) -> Void) {
    ...
    func updateProviderTokenAccounts(_ accounts: [UsageProvider: ProviderTokenAccountData]) {
        let summary = accounts
            ...
            .joined(separator: ",")
        UsagerLog.logger(LogCategories.tokenAccounts).info(
    ...
    func reloadConfig(reason: String) {
        ...
        do {
            ...
            UsagerLog.logger(LogCategories.configStore).error("Failed to reload config: \(error)")
        }
    }

    func applyExternalConfig(_ config: UsagerConfig, reason: String) {
        guard !self.configLoading else { return }
    ...
    private func bumpConfigRevision(_ context: ConfigChangeContext) {
        ...
        UsagerLog.logger(LogCategories.settings)
    ...
    func schedulePersistConfig() {
        ...
        if Self.isRunningTests {
            do {
                ...
                UsagerLog.logger(LogCategories.configStore).error("Failed to persist config: \(error)")
            }
            return
        }
