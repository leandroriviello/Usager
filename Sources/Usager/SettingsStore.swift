...
final class SettingsStore {
    ...
    @ObservationIgnored let configStore: UsagerConfigStore
    @ObservationIgnored let antigravityOAuthCredentialsStore: AntigravityOAuthCredentialsStore
    @ObservationIgnored var config: UsagerConfig
    ...
    #endif
    @ObservationIgnored var mergedMenuLastSelectedWasOverviewStorage = true
    ...
    init(
        ...
        configStore: UsagerConfigStore = UsagerConfigStore(),
        ...
        performInitialProviderDetection: Bool = !SettingsStore.isRunningTests)
    {
        ...
        if !Self.isRunningTests {
            UsagerLog.logger(LogCategories.settings).info(
                "App group resolved",
                metadata: [
                    "groupID": appGroupID,
        ...
        let legacyStores = UsagerConfigMigrator.LegacyStores(
        ...
        let config = UsagerConfigMigrator.loadOrMigrate(
        ...
        UsagerLog.setFileLoggingEnabled(self.debugFileLoggingEnabled)
...
extension SettingsStore {
    ...
    private static func loadDefaultsState(userDefaults: UserDefaults) -> SettingsDefaultsState {
        ...
        let debugLogLevelRaw = userDefaults.string(forKey: "debugLogLevel") ?? UsagerLog.Level.verbose.rawValue
        ...
        let mergedMenuLastSelectedWasOverview = userDefaults.object(
            forKey: "mergedMenuLastSelectedWasOverview") as? Bool ?? true
...
extension SettingsStore {
    var configSnapshot: UsagerConfig {
        _ = self.configRevision
        return self.config
    }

    func updateProviderState(config: UsagerConfig) {
    ...
    func setProviderEnabled(provider: UsageProvider, metadata _: ProviderMetadata, enabled: Bool) {
        UsagerLog.logger(LogCategories.settings).debug(
