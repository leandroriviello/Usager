...
private enum KeychainPromptMessage {
    static let browserCookie =
        "Usager will ask macOS Keychain for “%@” so it can decrypt browser cookies " +
        "and authenticate your account. Click OK to continue."

    static let claudeOAuth =
        "Usager will ask macOS Keychain for the Claude Code OAuth token " +
        "so it can fetch your Claude usage. Click OK to continue."
    static let codexCookie =
        "Usager will ask macOS Keychain for your OpenAI cookie header " +
        "so it can fetch Codex dashboard extras. Click OK to continue."
    static let claudeCookie =
        "Usager will ask macOS Keychain for your Claude cookie header " +
        "so it can fetch Claude web usage. Click OK to continue."
    static let cursorCookie =
        "Usager will ask macOS Keychain for your Cursor cookie header " +
        "so it can fetch usage. Click OK to continue."
    static let openCodeCookie =
        "Usager will ask macOS Keychain for your OpenCode cookie header " +
        "so it can fetch usage. Click OK to continue."
    static let factoryCookie =
        "Usager will ask macOS Keychain for your Factory cookie header " +
        "so it can fetch usage. Click OK to continue."
    static let zaiToken =
        "Usager will ask macOS Keychain for your z.ai API token " +
        "so it can fetch usage. Click OK to continue."
    static let syntheticToken =
        "Usager will ask macOS Keychain for your Synthetic API key " +
        "so it can fetch usage. Click OK to continue."
    static let copilotToken =
        "Usager will ask macOS Keychain for your GitHub Copilot token " +
        "so it can fetch usage. Click OK to continue."
    static let kimiToken =
        "Usager will ask macOS Keychain for your Kimi auth token " +
        "so it can fetch usage. Click OK to continue."
    static let kimiK2Token =
        "Usager will ask macOS Keychain for your Kimi K2 API key " +
        "so it can fetch usage. Click OK to continue."
    static let minimaxCookie =
        "Usager will ask macOS Keychain for your MiniMax cookie header " +
        "so it can fetch usage. Click OK to continue."
    static let minimaxToken =
        "Usager will ask macOS Keychain for your MiniMax API token " +
        "so it can fetch usage. Click OK to continue."
    static let augmentCookie =
        "Usager will ask macOS Keychain for your Augment cookie header " +
        "so it can fetch usage. Click OK to continue."
    static let ampCookie =
        "Usager will ask macOS Keychain for your Amp cookie header " +
        "so it can fetch usage. Click OK to continue."
}
...
enum KeychainPromptCoordinator {
    ...
    private static let log = UsagerLog.logger(LogCategories.keychainPrompt)
    ...
    static func disableKeychainForUnbundledExecutableIfNeeded() {
        ...
        guard Self.isUnbundledUsagerExecutable(executablePath) else { return }
        KeychainAccessGate.forceDisabledForProcess(reason: "unbundled-executable")
        Self.log.warning(
            "Unbundled Usager executable detected; disabling keychain access to avoid repeated prompts",
            metadata: ["doc": "docs/DEVELOPMENT_SETUP.md"])
    }

    static func isUnbundledUsagerExecutable(_ executablePath: String) -> Bool {
        guard executablePath.hasPrefix("/") else { return false }
        let executableURL = URL(fileURLWithPath: executablePath).standardizedFileURL
        return executableURL.lastPathComponent == "Usager"
