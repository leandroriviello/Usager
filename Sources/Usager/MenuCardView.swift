...
private struct MetricRow: View {
    ...
    var body: some View {
        ...
        .modifier(UsageMetricCardSurface(isEnabled: self.metric.cardStyle, isHighlighted: self.isHighlighted))
    }
}

private struct UsageMetricCardSurface: ViewModifier {
    let isEnabled: Bool
    let isHighlighted: Bool
    ...
    func body(content: Content) -> some View {
        if self.isEnabled {
            content
                .usagerMenuCardSurface(cornerRadius: 8)
                .overlay {
                    RoundedRectangle(cornerRadius: 8, style: .continuous)
                        .strokeBorder(Color.primary.opacity(self.isHighlighted ? 0.12 : 0.06), lineWidth: 0.5)
                }
        } else {
            content
        }
