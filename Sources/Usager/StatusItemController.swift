...
final class StatusItemController: NSObject, NSMenuDelegate, StatusItemControlling {
    ...
    private nonisolated static let statusItemAccessibilityTitle = "Usager"
    private nonisolated static let debugStatusItemAccessibilityTitle = "Usager Debug"
    private nonisolated static let statusItemAccessibilityIdentifierPrefix = "Usager.StatusItem"
    ...
    let loginLogger = UsagerLog.logger(LogCategories.login)
    let menuLogger = UsagerLog.logger(LogCategories.app)
    ...
    @objc private func handleProviderConfigDidChange(_ notification: Notification) {
        ...
        {
            if let config = notification.userInfo?["config"] as? UsagerConfig {
