...
final class OpenAICreditsPurchaseWindowController: NSWindowController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    private static let logHandlerName = "usagerLog"
    private static let debugLogURL = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("usager-buy-credits.log")
    private static let autoStartScript = """
    (() => {
      if (window.__usagerAutoBuyCreditsStarted) return 'already';
      const log = (...args) => {
        try {
          window.webkit?.messageHandlers?.usagerLog?.postMessage(args);
        } catch {}
      };
      ...
      const startNextPolling = (initialDelay = 500, interval = 500, maxAttempts = 90) => {
        if (window.__usagerNextPolling) return;
        window.__usagerNextPolling = true;
      ...
      const observeNextButton = () => {
        if (window.__usagerNextObserver || !window.MutationObserver) return;
        const observer = new MutationObserver(() => {
          if (clickNextIfReady(1)) {
            observer.disconnect();
            window.__usagerNextObserver = null;
          }
        });
        observer.observe(document.body, { subtree: true, childList: true, attributes: true });
        window.__usagerNextObserver = observer;
      };
      ...
      if (findAndClick()) {
        window.__usagerAutoBuyCreditsStarted = true;
        startNextPolling();
        observeNextButton();
        logDialogButtons();
        return 'clicked';
    ...
    private let logger = UsagerLog.logger(LogCategories.creditsPurchase)
