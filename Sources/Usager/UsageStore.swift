...
final class UsageStore {
    ...
    @ObservationIgnored private let sessionQuotaLogger = UsagerLog.logger(LogCategories.sessionQuota)
    @ObservationIgnored let openAIWebLogger = UsagerLog.logger(LogCategories.openAIWeb)
    @ObservationIgnored private let tokenCostLogger = UsagerLog.logger(LogCategories.tokenCost)
    @ObservationIgnored let augmentLogger = UsagerLog.logger(LogCategories.augment)
    @ObservationIgnored let providerLogger = UsagerLog.logger(LogCategories.providers)
    @ObservationIgnored let adaptiveRefreshLogger = UsagerLog.logger(LogCategories.adaptiveRefresh)
...
extension UsageStore {
    func debugDumpClaude() async {
        ...
        let url = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("usager-claude-probe.txt")
    ...
    func dumpLog(toFileFor provider: UsageProvider) async -> URL? {
        ...
        let filename = "usager-\(provider.rawValue)-probe.txt"
        ...
        fallbackCookieHeader: String) async -> ClaudeDebugLogConfiguration
    {
        await MainActor.run {
            ...
            return ClaudeDebugLogConfiguration(
                runtime: UsagerCore.ProviderRuntime.app,
