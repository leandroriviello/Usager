...
struct UsagerConfigMigrator {
    ...
    static func loadOrMigrate(
        ...
        stores: LegacyStores) -> UsagerConfig
    {
        let log = UsagerLog.logger(LogCategories.configMigration)
        let existing = try? configStore.load()
        var config = (existing ?? UsagerConfig.makeDefault()).normalized()
    ...
    private static func applyProviderOrder(_ raw: [String], config: UsagerConfig) -> UsagerConfig {
    ...
    private static func applyProviderToggles(
        ...
        config: UsagerConfig) -> UsagerConfig
    {
        var updated = config
