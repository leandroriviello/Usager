    ...
    static func mapUserFacingError(_ error: Error) -> CodexSystemAccountPromotionUserFacingError {
        ...
        if let error = error as? CodexAccountPromotionError {
            ...
            case .targetManagedAccountNotFound:
                L("That account is no longer available in Usager. Refresh the account list and try again.")
            case .targetManagedAccountAuthMissing:
                L("Usager could not find saved auth for that account. Re-authenticate it and try again.")
            case .targetManagedAccountAuthUnreadable:
                L("Usager could not read saved auth for that account. Re-authenticate it and try again.")
            case .liveAccountUnreadable:
                L("Usager could not read the current system account on this Mac.")
            case .liveAccountMissingIdentityForPreservation:
                L("Usager could not safely preserve the current system account before switching.")
            case .liveAccountAPIKeyOnlyUnsupported:
                L("Usager can't replace a system account that is signed in with an API key only setup.")
            case .displacedLiveManagedAccountConflict:
                L(
                    "Usager found another managed account that already uses the current system account. " +
                        "Resolve the duplicate account before switching.")
            case .displacedLiveImportFailed:
                L("Usager could not save the current system account before switching.")
            case .managedStoreCommitFailed:
                L("Usager could not update managed account storage.")
            case .liveAuthSwapFailed:
                L("Usager could not replace the live Codex auth on this Mac.")
            }
