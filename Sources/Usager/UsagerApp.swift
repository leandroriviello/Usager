...
struct UsagerApp: App {
    ...
    init() {
        ...
        let storedLevel = UsagerLog.parseLevel(UserDefaults.standard.string(forKey: "debugLogLevel")) ?? .verbose
        let level = UsagerLog.parseLevel(env["USAGER_LOG_LEVEL"]) ?? storedLevel
        UsagerLog.bootstrapIfNeeded(.init(
            destination: .oslog(subsystem: "com.leandroriviello.usager"),
            level: level,
            json: false))
        ...
        UsagerLog.logger(LogCategories.app).info(
            "Usager starting",
            metadata: [
                "version": version,
                "build": build,
        ...
        UsagerLog.setLogLevel(settings.debugLogLevel)
    ...
    var body: some Scene {
        ...
        WindowGroup("UsagerLifecycleKeepalive") {
            HiddenWindowView()
        }
...
private func makeUpdaterController() -> UpdaterProviding {
    ...
    if InstallOrigin.isHomebrewCask(appBundleURL: bundleURL) {
        return DisabledUpdaterController(
            unavailableReason: "Updates managed by Homebrew. Run: brew upgrade --cask steipete/tap/usager")
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let confettiLogger = UsagerLog.logger(LogCategories.confetti)
    ...
    private func ensureStatusController() {
        ...
        UsagerLog.logger(LogCategories.app)
    ...
    private func installDebugMemoryPressureObserverIfNeeded() {
        ...
        self.debugMemoryPressureObserver = DistributedNotificationCenter.default().addObserver(
            forName: .usagerDebugSimulateMemoryPressure,
            object: nil,
            queue: .main)
    ...
    private func handleDebugMemoryPressureNotification(rawLevel: String?, shouldSeedCaches: Bool) {
        ...
        UsagerLog.logger(LogCategories.memoryPressure).info(
